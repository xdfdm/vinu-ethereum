...
func (srv *Server) postHandshakeChecks(peers map[enode.ID]*Peer, inboundCount int, c *conn) error {
	...
	case discfilter.BannedDynamic(c.node.ID()):
		...
		srv.log.Debug("Dropping banned peer", "peer", c, "node_id", c.node.ID())
		srv.log.Debug("discfilter", "banned", discfilter.GetBanned())
		return DiscBanned
...
func (srv *Server) addPeerChecks(peers map[enode.ID]*Peer, inboundCount int, c *conn) error {
	...
	if len(srv.Protocols) > 0 && countMatchingProtocols(srv.Protocols, c.caps) == 0 {
		srv.log.Debug("Dropping peer with no matching protocols", "peer", c, "caps", c.caps, "len(Protocols)", len(srv.Protocols), "Protocols", srv.Protocols, "countMatchingProtocols", countMatchingProtocols(srv.Protocols, c.caps))
		srv.log.Debug("Ban node with no matching protocols", "node", c.node.String(), "node_id", c.node.ID())
